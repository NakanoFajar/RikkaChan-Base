...
//nocache
nocache('./wibuxyz.js', module => console.log(color('|UPDATE BASE|'), color(`${module} Updated!`, 'red')))
...
const starts = async (helga = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('RikkaChan Base', {
		font: 'Standard',
	...
	console.log(color('\n> Private Base ','silver'), color(`🗿🤙`,'mediumseagreen'))
console.log(color('> Support RikkaChan ','silver'), color(`🗿🤙`,'mediumseagreen'))
console.log(color('> Anjay Alok ','silver'), color(`🗿🤙`,'mediumseagreen'))
	helga.browserDescription = [ 'Wibu Base By FajarrXd', 'ubuntu', '3.0' ]

	helga.on('qr', () => {
	console.log(color('[','white'), color('!','red'), color(']','white'), color('Helga Base'))
})
	helga.on('credentials-updated', () => {
		fs.writeFileSync('./src/session.json', JSON.stringify(helga.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'Base Helga')
	})
	fs.existsSync('./src/session.json') && helga.loadAuthInfo('./src/session.json')
	helga.on('connecting', () => {
		start('2', 'NewBase')
	})
	helga.on('open', () => {
...
contentText: `${masuk}`,
footerText: `_Created By ٬࿊⃟RikkaChan シ_︎`, 
buttons: gbutsan,
headerType: 4 }
...
const buttonMessages = { imageMessage: mhan.message.imageMessage,
contentText: `${keluar}`,
footerText: `_Created By ٬࿊⃟RikkaChan シ︎_`,
buttons: gbutsan,
headerType: 4 }
			helga.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, { thumbnail: fs.readFileSync('./helga.jpg'), "contextInfo": { mentionedJid: [num]}, caption: 'Tes', quoted: fkontakk})
