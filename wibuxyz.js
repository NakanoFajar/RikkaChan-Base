...
const dtod = "6283871194301@s.whatsapp.net"
const otod = "6283871194301@s.whatsapp.net"
...
nopref = false
    ...
    //── 「 LolKey 」 ──//
thumbz = fs.readFileSync('./image/thumb.jpg')
...
try {
		...
		const dfrply = fs.readFileSync('./image/fake.jpg')
		cmhit.push(command)
        mess = {
        ...
        const grupinv = (teks) => {
        	grup = helga.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6283871194301-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/IlpQRKWDZed0GgVHmYJUbl', "groupName": `${NamaBot}`, "footerText": "*_© Helga Zex_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
await helga.setStatus(`Copyright RikkChan`).catch((_) => _);
      settingstatus = new Date() * 1;
		...
		const pporigi = fs.readFileSync('./image/fake.jpg')
		...
		message: {
			"productMessage": {
				"product": {
					"productImage":{
						"mimetype": "image/jpeg",
						"jpegThumbnail": thumbz //Gambarnye
					},
...
const finv = {
	...
	"message": {
		"groupInviteMessage": {
			"groupJid": "6283871194301-1616169743@g.us",
...
`
sendButLocation(from, `${menu}`, "*RikkaChan*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
break
     case 'menu':
  if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
        ptod = "6283871194301@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
...
╭─「 Info RikkaChan 」
│ • Status : ${isOwner ? 'Owner' : 'User RikkaChan'}
│ • Nama : ${pushname}
│ • Nomor : @${stod.split('@')[0]}
...
╰────
`
sendButLocation(from, `${menu}`, `${uwu}© RikkaChan 2022${uwu}`, {jpegThumbnail:thumbz}, [{buttonId:`${prefix}about`,buttonText:{displayText:'ABOUT🌱'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'bugreport':
              if (args.length < 1) return reply(`Ketik ${prefix}bugreport [fiturnya] [Error Nya Gimana]`) 
              teks = args.join(' ')
              reply('Terima Kasih Telah Melaporkan Bug Pada Owner, Jika Itu Sekedar Iseng Maka Akan Di Ban Oleh Bot!')
              helga.sendMessage('6283871194301@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
              case 'verify':
...
helga.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./image/fake.jpg')})
break
case  'blowjob':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
Giliran = @${tty.player1.split('@')[0]}`
 helga.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     helga.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Support Owner RikkaChan*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer RikkaChan",body:"",previewType:"Wibu Base",thumbnail:fs.readFileSync('./image/fake.jpg'),sourceUrl:"https://wa.me/6283871194301"}}})
	}
    }
    }
    }
